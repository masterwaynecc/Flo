...
final class LocalStore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "app.luma.localstore")

    init(filename: String = "luma-state.json") {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? FileManager.default.temporaryDirectory
        let folder = dir.appendingPathComponent("Luma", isDirectory: true)
    ...
    func load() -> PersistedState {
        queue.sync {
            guard let data = try? Data(contentsOf: fileURL),
                  let state = try? JSONDecoder.luma.decode(PersistedState.self, from: data) else {
    ...
    func save(profile: UserProfile, dayLogs: [DayLog]) {
        queue.sync {
            ...
            guard let data = try? JSONEncoder.luma.encode(state) else { return }
