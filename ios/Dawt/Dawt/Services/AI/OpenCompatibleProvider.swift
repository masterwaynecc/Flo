...
struct OpenCompatibleProvider: AIProvider {
    ...
    static func ollama(
        ...
        model: String = ProcessInfo.processInfo.environment["DAWT_OSS_MODEL"] ?? "llama3.2"
    ...
    static func selfHosted(
        baseURL: URL = URL(string: ProcessInfo.processInfo.environment["DAWT_OSS_BASE_URL"] ?? "http://127.0.0.1:8000/v1")!,
        apiKey: String? = ProcessInfo.processInfo.environment["DAWT_OSS_API_KEY"],
        model: String = ProcessInfo.processInfo.environment["DAWT_OSS_MODEL"] ?? "llama3.2"
    ) -> OpenCompatibleProvider {
