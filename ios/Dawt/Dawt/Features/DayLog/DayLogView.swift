                        ...
                        HStack(spacing: 8) {
                            ForEach(LogCategory.allCases) { item in
                                Button(item.title) { category = item }
                                    ...
                                    .padding(.vertical, 8)
                                    .background(category == item ? DawtColor.rose : Color.white.opacity(0.7), in: Capsule())
                                    .foregroundStyle(category == item ? Color.white : DawtColor.ink)
                            }
                        }
                ...
                Button {
                    ...
                    VStack(spacing: 8) {
                        Image(systemName: level == .none ? "circle" : "drop.fill")
                            .font(.title2)
                            .foregroundStyle(level == .none ? DawtColor.inkMuted : DawtColor.period)
                        Text(level.title)
                            .font(DawtType.body(15, weight: .medium))
                    }
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 20)
                    .background(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                            .fill(flow == level ? DawtColor.rose.opacity(0.18) : Color.white.opacity(0.75))
                    )
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                            .stroke(flow == level ? DawtColor.rose : Color.clear, lineWidth: 2)
                    )
                }
                .buttonStyle(.plain)
            }
                        ...
                        ForEach(grouped[key] ?? []) { item in
                            ...
                            Button {
                                ...
                                Text(item.title)
                                    ...
                                    .padding(.vertical, 8)
                                    .background(on ? DawtColor.rose : Color.white.opacity(0.75), in: Capsule())
                                    .foregroundStyle(on ? Color.white : DawtColor.ink)
                            }
                            .buttonStyle(.plain)
                        }
                    }
